...
int main(int argc, char* argv[]) {
    ...
    while((n = getopt(argc, argv, "vdh")) != -1) {  
        if (n=='v') {
            ...
            printf("\t%s\n", "");
            printf("\t%s\n", "./next-pass [-v] [-d] [-h] REQUEST_PORT MY_PORT [NEXT_PORT]");
            printf("\t%s\n", "===========================================================");
            printf("\t%s\n", "");
            printf("\t%s\n", "OPTIONS:");
            ...
            printf("\t%s\n", "");
            printf("\t%s\n", "ARGUMENTS:");
            printf("\t%s\n", "REQUEST_PORT\t- the port for the container to which this program is requesting a shut down");
    ...
    while (1) {
        ...
        if (n==-1&&(errno==EAGAIN||errno==EWOULDBLOCK)) {
            ...
            } else if (buffer==CLOSING_MSG&&ntohs(client_addr.sin_port)==c_port) {
